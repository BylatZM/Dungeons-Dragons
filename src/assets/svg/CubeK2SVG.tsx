...
export const CubeK2SVG: FC<IProps> = ({ clicksNumber, changeClicksNumber }) => {
	return (
		<button
			className='relative min-w-[70px] h-[70px] rounded-full bg-black flex justify-center items-center'
			onClick={() => changeClicksNumber(prev => ({ ...prev, K2: prev.K2 + 1 }))}
		>
			<div
				className={clsx(
					...
					clicksNumber.K2 === 0 ? 'opacity-0' : 'opacity-100'
				)}
			>
				{clicksNumber.K2}
			</div>
			<div className='flex flex-col items-center gap-y-1'>
				...
				<span className='text-xs text-white'>K2</span>
			</div>
		</button>
	)
}
