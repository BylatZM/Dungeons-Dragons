...
export const CubeK4SVG: FC<IProps> = ({ clicksNumber, changeClicksNumber }) => {
	return (
		<button
			className='relative min-w-[70px] h-[70px] rounded-full bg-black flex justify-center items-center'
			onClick={() => changeClicksNumber(prev => ({ ...prev, K4: prev.K4 + 1 }))}
		>
			<div
				className={clsx(
					...
					clicksNumber.K4 === 0 ? 'opacity-0' : 'opacity-100'
				)}
			>
				{clicksNumber.K4}
			</div>
			<div className='flex flex-col items-center gap-y-1'>
				...
				<span className='text-xs text-white'>K4</span>
			</div>
		</button>
	)
}
